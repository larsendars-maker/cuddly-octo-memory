...
## Глобальный доступ
В Render добавь `ALLOW_ALL_COUNTRIES=true`. OrbitDesk не выполняет country/IP geoblock; IP может использоваться только для rate limits и антибот-защиты. Если доступ ограничивается снаружи приложения (например, Cloudflare/WAF/корпоративная сеть), это настраивается отдельно.

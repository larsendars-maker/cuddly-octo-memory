...
export async function initDb() {
  ...
  await pool.query(`
    ...
    create table if not exists sessions (id bigserial primary key, token_hash char(64) unique not null, user_id integer not null references users(id) on delete cascade, expires_at timestamptz not null, privileged_at timestamptz, created_at timestamptz not null default now());
...
create table if not exists visits (id bigserial primary key, user_id integer not null references users(id) on delete cascade, url text not null, title varchar(200) not null default '', visited_at timestamptz not null default now());
  ...
  await pool.query(`alter table sessions add column if not exists privileged_at timestamptz`);
...
function memQ(text, params=[]) {
  ...
  if (s.startsWith('select m.id,m.sender_id,m.recipient_id,m.body,m.read_at,m.created_at,u.username as sender_name from messages')) { const a=Number(p(1)),b=Number(p(2)); const rows=mem.messages.filter(m=>(m.sender_id===a&&m.recipient_id===b)||(m.sender_id===b&&m.recipient_id===a)).sort((x,y)=>x.id-y.id).slice(-120).map(m=>({...m,read_at:m.read_at||null,sender_name:mem.users.find(u=>u.id===m.sender_id)?.username||'user'})); return result(rows); }
  if (s.startsWith('insert into messages(')) { const r={id:uid('messages'),sender_id:Number(p(1)),recipient_id:Number(p(2)),body:p(3),read_at:null,created_at:now().toISOString()};mem.messages.push(r);return result([clone(r)]); }

  if (s.startsWith('insert into sessions(')) { const expires=new Date(Date.now()+14*86400000).toISOString(); const r={id:uid('sessions'),token_hash:p(1),user_id:Number(p(2)),expires_at:expires,privileged_at:p(4)||null,created_at:now().toISOString()};mem.sessions.push(r);return result([]); }
  ...
  if (s.startsWith('select u.id,u.username,u.email,u.xp,u.role,u.email_verified,u.email_verified_at,u.created_at,u.blocked,u.block_reason,s.privileged_at from sessions s join users u on u.id=s.user_id') || s.startsWith('select u.id,u.username,u.email,u.xp,u.role,u.created_at,u.blocked,u.block_reason from sessions s join users u on u.id=s.user_id')) { const srow=mem.sessions.find(x=>x.token_hash===p(1)&&new Date(x.expires_at)>new Date()); const u=srow?mem.users.find(x=>x.id===srow.user_id):null; return result(u?[{...clone(u),privileged_at:srow?.privileged_at||null}]:[]); }
  if (s.startsWith('update sessions set privileged_at=now() where token_hash=$1')) { const row=mem.sessions.find(x=>x.token_hash===p(1)); if(!row||new Date(row.expires_at)<=new Date()) return result([]); row.privileged_at=now().toISOString(); return result([{id:row.id}]); }
  if (s.startsWith('update sessions set privileged_at=null where token_hash=$1')) { const row=mem.sessions.find(x=>x.token_hash===p(1)); if(!row) return result([]); row.privileged_at=null; return result([{id:row.id}]); }
